 ...
 *
 * Usage on prod:
 *   cd /opt/itmen-pipeline/api && node tools/restore-from-audit.js [--since ISO] [--dry-run] [--apply]
 */

const { listAll, findOne } = require("../src/pb-client");
const { loadPipelineState, upsertDeal } = require("../src/mapper");
...
function hasMeaningfulAuditValue(field, val) {
  ...
  if (field === "projectTasks" && Array.isArray(val)) {
    return val.some(t => t && String(t).trim());
  }
  return true;
}

function segmentHasContent(field, val) {
  if (val == null) return false;
  if (field === "changePains") return String(val).trim().length > 0;
  if (field === "asIsStack" && typeof val === "object") {
    return Boolean(val.vendor || val.product || val.comment);
  }
  if (field === "competitorEntries" && Array.isArray(val)) {
    return val.some(e => e && (e.vendor || e.product));
  }
  return !isEmpty(val);
}
...
function shouldRestore(field, auditVal, currentVal) {
  ...
  if (field === "projectTasks" && Array.isArray(auditVal) && Array.isArray(currentVal)) {
    return auditVal.some(t => t && !currentVal.includes(t));
  }
  if (typeof auditVal === "object" && typeof currentVal === "object" && !Array.isArray(auditVal)) {
    for (const [k, v] of Object.entries(auditVal)) {
      ...
      if (segmentHasContent(field, v) && !segmentHasContent(field, cur)) return true;
    }
  }
  return false;
}
...
function mergeField(field, auditVal, currentVal) {
  ...
  if (typeof auditVal === "object" && typeof currentVal === "object" && !Array.isArray(auditVal)) {
    ...
    for (const [k, v] of Object.entries(auditVal)) {
      if (!segmentHasContent(field, out[k]) && segmentHasContent(field, v)) out[k] = v;
...
async function main() {
  ...
  console.log(`Audit rows total: ${rows.length}, by managers: ${filtered.length}`);
  ...
  for (const [key, row] of Object.entries(byDealField)) {
    ...
    let deal;
    try {
      deal = await loadPipelineState({ dealId });
    } catch (e) {
    ...
    if (!dealPatches[dealId]) {
      dealPatches[dealId] = { customer: row.customer, savedBy: row.saved_by, fields: {} };
    }
  ...
  console.log(`\n=== RESTORE CANDIDATES: ${report.length} field(s) on ${Object.keys(dealPatches).length} deal(s) ===`);
  for (const r of report) {
    console.log(`\n${r.dealId} ${r.customer} [author: ${r.savedBy} @ ${r.at}]`);
    console.log(`  ${r.label}:`);
    console.log(`    audit:   ${r.auditPreview}`);
    console.log(`    current: ${r.currentPreview}`);
    console.log(`    restore: ${r.restorePreview}`);
  }

  if (!opts.apply) {
    console.log(`\nDry run. Re-run with --apply to write ${Object.keys(dealPatches).length} deal(s).`);
    return;
  }
