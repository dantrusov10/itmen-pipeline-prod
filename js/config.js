...
window.ITMEN_CONFIG = {
  scoreCriteria: [
    { key: "loyalty", name: "Лояльность клиента к нам", weight: 0.08, question: "Насколько клиент нам лоялен, доверяет нам и готов продвигать нас внутри?", manualOnly: true },
    { key: "commit", name: "Подтверждение коммита клиента", weight: 0.08, question: "Насколько клиент подтвердил намерение участвовать в закупке?" },
    { key: "budget", name: "Определённость бюджета", weight: 0.144, question: "Насколько понятен и подтверждён бюджет на проект?" },
    { key: "fit", name: "Соответствие проблеме", weight: 0.144, question: "Насколько ITMen закрывает заявленные боли и выбранные сегменты?" },
    { key: "timing", name: "Срочность сроков", weight: 0.112, question: "Когда клиент готов принять решение?" },
    { key: "competitive", name: "Конкурентная позиция", weight: 0.08, question: "Насколько мы конкурентны в этой сделке?" },
    { key: "access", name: "Доступ / влияние", weight: 0.064, question: "Есть ли доступ к ЛПР и понятна ли карта влияния?" },
    { key: "technical", name: "Техн. соответствие", weight: 0.048, question: "Насколько продукт закрывает % требований проекта и пилота?" },
    { key: "commercial", name: "Коммерч. готовность клиента", weight: 0.048, question: "Насколько клиент готов к закупке: процесс, сроки, участники?" },
    { key: "manualProb", name: "Вероятность (оценка менеджера)", weight: 0.20, question: "Субъективная оценка менеджера: насколько реалистично закрытие сделки (0–100%)?", manualOnly: true },
  ],

  manualScoreKeys: ["loyalty", "manualProb"],
  ...
  scoreRubrics: {
    ...
    manualProb: {
      s5: "90–100% — менеджер уверен в закрытии",
      s4: "70–89% — высокая вероятность",
      s3: "50–69% — реалистичный шанс",
      s2: "30–49% — под вопросом",
      s1: "10–29% — маловероятно",
      s0: "0–9% / не оценена",
    },
  },
...
function getMergedScoringItems(scoringSource) {
  ...
  const keyHints = {
    ...
    technical: "Техн", commercial: "Коммерч", manualProb: "Вероятность",
  };
